...
void omp_lu_avx128_dynamic(float mat[][N], int n, int thread_count) {
    __m128 t1, t2, t3;
    #pragma omp parallel num_threads(thread_count)
    for (int k = 0; k < n; k++) {
        float temp1[4] = {mat[k][k], mat[k][k], mat[k][k], mat[k][k]};
...
void omp_static_sse_dynamicThreads(float A[N][N], int n,int NUM_THREADS) {
...
int main() {
    ...
    for (i = 2000; i <= 2000; i += s) {
        ...
        init_mat(m, i);
           ...
           // print_mat(mat);
        for (int count = 4; count <= 20; count += 4) {
            cout << "线程数： " << count << endl;
            ...
            QueryPerformanceCounter((LARGE_INTEGER *)&head); // start time
          //  for(int x=0;x<5;x++)
            omp_static_sse_dynamicThreads(mat, i, count);
            QueryPerformanceCounter((LARGE_INTEGER *)&tail); // end time
            cout << "sse and dynamicThreads and static OpenMP LU: " << (tail - head) * 1000.0 / (freq)<< "ms" << endl;
         //    cout << "sse and dynamic OpenMP LU: " << (tail - head) * 1000.0 / (freq*5)<< "ms" << endl;
          ...
          //  for(int x=0;x<5;x++)
            ...
            QueryPerformanceCounter((LARGE_INTEGER *)&tail); // end time
            cout << "static sse OpenMP LU: " << (tail - head) * 1000.0 / (freq)<< "ms" << endl;
         //    cout << "static sse OpenMP LU: " << (tail - head) * 1000.0 / (freq*5)<< "ms" << endl;
          //  print_mat(mat);
