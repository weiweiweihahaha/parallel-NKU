...
//           // cout << "sse and static OpenMP LU: " << (tail - head) * 1000.0 /(freq*5)<< "ms" << endl;
         ...
         //    cout << "sse and dynamic OpenMP LU: " << (tail - head) * 1000.0 / (freq*5)<< "ms" << endl;
          //  print_mat(mat);
            ...
            QueryPerformanceCounter((LARGE_INTEGER *)&head); // start time
          //  for(int x=0;x<5;x++)
            omp_lu_avx128_dynamic(mat, i, count);
            QueryPerformanceCounter((LARGE_INTEGER *)&tail); // end time
            cout << "avx and dynamic OpenMP LU: " << (tail - head) * 1000.0 / (freq)<< "ms" << endl;
         //    cout << "avx and dynamic OpenMP LU: " << (tail - head) * 1000.0 / (freq*5)<< "ms" << endl;
          //  print_mat(mat);
