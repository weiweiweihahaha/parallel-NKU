...
void omp_lu_sse_dynamic(float mat[][N], int n, int thread_count,int chunk_size) {
    __m128 t1, t2, t3;
    #pragma omp parallel num_threads(thread_count)
    for (int k = 0; k < n; k++) {
...
void omp_lu_guided_sse(float mat[][N], int n, int thread_count,int chunk_size) {
    __m128 t1, t2, t3;
    #pragma omp parallel num_threads(thread_count)
    for (int k = 0; k < n; k++) {
        float temp1[4] = {mat[k][k], mat[k][k], mat[k][k], mat[k][k]};
...
//           // cout << "sse and static OpenMP LU: " << (tail - head) * 1000.0 /(freq*5)<< "ms" << endl;
        ...
        cout << "chunk_size： " << size[2] << endl;

            reset_mat(mat, m, i);
            QueryPerformanceCounter((LARGE_INTEGER *)&head); // start time
            for(int x=0;x<5;x++)
            omp_lu_sse_dynamic(mat, i, count,size[2]);
            QueryPerformanceCounter((LARGE_INTEGER *)&tail); // end time
...
//            QueryPerformanceCounter((LARGE_INTEGER *)&head); // start time
//            for(int x=0;x<5;x++)
//            omp_lu_guided_sse(mat, i, count,size[0]);
//            QueryPerformanceCounter((LARGE_INTEGER *)&tail); // end time
